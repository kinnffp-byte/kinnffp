...
-- ── [추가 패치 8] 히어로 로고 ────────────────────────────────────
update profile set data = data || jsonb_build_object(
         'hero-logo','',
         'cover-logo','assets/logo-wordmark-white.png')
 where id = 1;
select data->>'hero-logo' as hero_logo, data->>'cover-logo' as cover_logo from profile where id = 1;

-- ── [추가 패치 9] 사이드바는 로고 대신 KIMFP 글자로 되돌림 ──────
update profile set data = jsonb_set(data, '{logo-image}', '""'::jsonb) where id = 1;
select data->>'logo-image' as sidebar_logo, data->>'brand-name' as sidebar_text
  from profile where id = 1;

-- ── [추가 패치 10] 히어로 우측 상단 로고 제거 ───────────────────
update profile set data = jsonb_set(data, '{hero-logo}', '""'::jsonb) where id = 1;
select data->>'hero-logo' as hero_logo, data->>'logo-image' as sidebar_logo,
       data->>'cover-logo' as cover_logo, data->>'logo-emblem' as emblem
  from profile where id = 1;

-- ── [추가 패치 11] 히어로 우측 로고 다시 켜기 ──────────────────
update profile set data = jsonb_set(data, '{hero-logo}', '"assets/logo-wordmark.png"'::jsonb)
 where id = 1;
select data->>'hero-logo' as hero, data->>'logo-image' as sidebar,
       data->>'cover-logo' as cover, data->>'logo-emblem' as emblem from profile where id=1;

-- ── [추가 패치 12] 글자 크기 배율 (기본 1 = 지금 그대로) ────────
update profile set data = data || jsonb_build_object('font-all','1', 'font-hero','1', 'font-title','1', 'font-sub','1', 'font-kicker','1', 'font-body','1', 'font-number','1', 'font-nav','1') where id = 1;
select data->>'font-all' as all, data->>'font-title' as title, data->>'font-body' as body
  from profile where id = 1;

-- ── [추가 패치 13] 룰렛 3열 (열당 6항목) ────────────────────────
update profile set data = jsonb_set(data,'{roulette-percol}','"6"'::jsonb) where id = 1;
select data->>'roulette-percol' as percol from profile where id = 1;

-- ── [추가 패치 14] 공약 문구 호룰 → 호랄로 ──────────────────────
update promises set title = replace(title,'호룰','호랄로') where title like '%호룰%';
select title from promises where amount = '90만';

-- ── [추가 패치 15] 페리윙클 팔레트 + 히어로 로고 제거 ───────────
update profile set data = data || jsonb_build_object('theme-ink','#26314D', 'theme-muted','#5A6B92', 'theme-paper','#FBFCFE', 'theme-sheet','#FFFFFF', 'theme-night','#5372BE', 'theme-night-soft','#6A85C9', 'theme-night-deep','#4A68B8', 'theme-hero','#6688CC', 'theme-iris','#3B5BB5', 'theme-ice','#DBE1FF', 'theme-rose','#F0DCEC', 'theme-sand','#F2C95A', 'theme-mint','#D5E6E2', 'theme-page','#DDE4F5', 'theme-screen','#FFFFFF') where id = 1;
update profile set data = jsonb_set(data, '{hero-logo}', '""'::jsonb) where id = 1;
select data->>'theme-night' as sidebar, data->>'theme-hero' as hero,
       data->>'theme-iris' as accent, data->>'hero-logo' as hero_logo
  from profile where id = 1;
